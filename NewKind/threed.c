...
{
	gfx_draw_circle (xo, yo, radius, 0x40);
}
...
//	radius = 6291456 / ship_vec.z;   /* Planets are BIG! */
	...
	if ((x + radius <  0) ||
		...
		(y - radius > 383))
		return; 
    gfx_draw_filled_circle (x, y, radius, 0x40);
/*
	switch (planet_render_style)
	{
		case 0:
...
void draw_sun (struct univ_object *planet)
{
    ...
    gfx_draw_filled_circle (x, y, radius, 0x7f);
    if (radius >20)
      gfx_draw_filled_circle (x, y, radius-20, 0x7f);
...
//	else
	...
	for (cnt = 0; cnt < np; cnt++)
	{
		...
		for (i = 0; i < /*16*/4; i++)
		{
			...
			sizex = ( (randint() & 1) + 1 )&0x7;
			sizey = ( (randint() & 1) + 1)&0x7;
