...
#define program_write_byte_8(addr,value)  write_byte(addr, value)
#define program_read_byte_8(addr) read_byte(addr)

#define io_read_byte_8(addr) io_read_byte(((unsigned char)addr))
#define io_write_byte_8(addr,value)  io_write_byte(((unsigned char)addr), value)

INLINE void io_write_byte(unsigned char addr, UINT8 value)
{
  psIoWrite = Z80.z80IoWrite; /* Beginning of our handler */
  while (psIoWrite->lowIoAddr != 0xffff)
  {
      if ((addr >= psIoWrite->lowIoAddr) && (addr <= psIoWrite->highIoAddr))
      {
          psIoWrite->IOCall(addr, value, psIoWrite);
          return;
      }
      ++psIoWrite;
  }
...
INLINE UINT8 io_read_byte(unsigned char addr)
{
  psIoRead = Z80.z80IoRead;   /* Beginning of our handler */
  while (psIoRead->lowIoAddr != 0xffff)
  {
      if ((addr >= psIoRead->lowIoAddr) && (addr <= psIoRead->highIoAddr))
      {
          return psIoRead->IOCall(addr, psIoRead);
      }
      ++psIoRead;
  }
  return 0xff; /* Unclaimed I/O read */
}

INLINE UINT8 read_byte(int addr)
{
  psMemRead = Z80.z80MemRead; /* Beginning of our handler */
  while ((psMemRead->lowAddr & 0xffffffff)!= 0xffffffff)
  {
      if ((addr >= psMemRead->lowAddr) && (addr <= psMemRead->highAddr))
      {
          if (psMemRead->memoryCall)
          {
              return psMemRead->memoryCall(addr, psMemRead);
          }
          else
          {
              return *((UINT8 *) psMemRead->pUserArea + (addr - psMemRead->lowAddr));
          }
      }
      ++psMemRead;
  }
  return Z80.z80Base[addr];
}

INLINE void write_byte(int addr, UINT8 value)
{
  
  psMemWrite = Z80.z80MemWrite;   /* Beginning of our handler */
  while ((psMemWrite->lowAddr & 0xffffffff)!= 0xffffffff)
  {
      if ((addr >= psMemWrite->lowAddr) && (addr <= psMemWrite->highAddr))
      {
          if (psMemWrite->memoryCall)
          {
              psMemWrite->memoryCall(addr, value, psMemWrite);
          }
          else
          {
              *((UINT8 *) psMemWrite->pUserArea + (addr - psMemWrite->lowAddr)) = value;
          }
          return;
      }
      ++psMemWrite;
  ...
  if (psMemWrite)
  {
      Z80.z80Base[addr] = (UINT8) value;
  }
}


#define cpu_readop(addr) (*(Z80.z80Base + (addr)))
#define cpu_readop_arg(addr) (*(Z80.z80Base + (addr)))
...
INLINE void BURNODD(int cycles, int opcodes, int cyclesum)
{
    if( cycles > 0 )
    {
        ...
        Z80.dwElapsedTicks += (cycles / cyclesum) * cyclesum;
    }
}
 ...
 ***************************************************************/
#define CC(prefix,opcode) do {z80_ICount -= cc[Z80_TABLE_##prefix][opcode];Z80.dwElapsedTicks += cc[Z80_TABLE_##prefix][opcode];} while (0)
...
static void take_interrupt(void)
{
//    printf("WARNING!!! INTERRUPT ISSUSED!!!\n\r");
    if( IFF1 )
    {
        int irq_vector=Z80.z80intAddr;
...
{
//    printf("WARNING!!! INTERRUPT ISSUSED = %08x!!!\n\r");
    if( IFF1 )
    {
        /* there isn't a valid previous program counter */
        PRVPC = -1;
...
// Malban, done by AAE init
// this init would fuck up pointers to read/write
...
int z80_execute(int cycles)
{
    ...
    Z80.dwElapsedTicks += Z80.extra_cycles;
...
{
    UINT32 dwTemp = Z80.dwElapsedTicks;
