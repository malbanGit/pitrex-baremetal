...
const char *pbIndexedRegPairs[4] = 
{
	"cx",	// BC
	"word [_z80de]", // DE
	"di",	// IX/IY
...
{
	fprintf(fp, "				InvalidInstruction(%d);\n", dwCount);
}
...
void SetSubFlagsSZHVC(const char *pszLeft, const char *pszRight)
{
...
//    fprintf(fp, "               cpu.z80F |= Z80_FLAG_NEGATIVE; // malban, for now I want to be certain\n");
}

void SetSbcFlagsSZHVC(const char *pszLeft, const char *pszRight)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
void SetAddFlagsSZHVC(const char *pszLeft, const char *pszRight)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
void SetAdcFlagsSZHVC(const char *pszLeft, const char *pszRight)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
void WriteValueToMemory(const char *pszAddress, const char *pszValue)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		fprintf(fp, "checkLoop%d:\n", dwGlobalLabel);
		fprintf(fp, "		cmp	[edi], word 0ffffh ; End of our list?\n");
		fprintf(fp, "		je	memoryWrite%d	; Yes - go write it!\n", dwGlobalLabel);
		...
		fprintf(fp, "		jb	nextAddr%d	; Yes... go to the next addr\n", dwGlobalLabel);
		...
		fprintf(fp, "		jbe	callRoutine%d	; If not, go call it!\n\n", dwGlobalLabel);
		fprintf(fp, "nextAddr%d:\n", dwGlobalLabel);
		fprintf(fp, "		add	edi, 10h		; Next structure, please\n");
		fprintf(fp, "		jmp	short checkLoop%d\n\n", dwGlobalLabel);
		fprintf(fp, "callRoutine%d:\n", dwGlobalLabel);
		...
		fprintf(fp, "		jmp	short WriteMacroExit%d\n", dwGlobalLabel);
	
		fprintf(fp, "memoryWrite%d:\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "						else\n");
		fprintf(fp, "						{\n");
		fprintf(fp, "							*((const char *) psMemWrite->pUserArea + (%s - psMemWrite->lowAddr)) = %s;\n", pszAddress, pszValue);
		fprintf(fp, "						}\n");
...
void WriteWordToMemory(const char *pszAddress, const char *pszTarget)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		fprintf(fp, "		je		memoryWrite%d\n", dwGlobalLabel);
		fprintf(fp, "		cmp	%s, [edi]	; Are we smaller?\n", pszAddress);
		fprintf(fp, "		jb		nextAddr%d		; Yes, go to the next address\n", dwGlobalLabel);
		fprintf(fp, "		cmp	%s, [edi+4]	; Are we bigger?\n", pszAddress);
		fprintf(fp, "		jbe	callRoutine%d\n\n", dwGlobalLabel);
		fprintf(fp, "nextAddr%d:\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "						{\n");
		fprintf(fp, "							*((const char *) psMemWrite->pUserArea + (%s - psMemWrite->lowAddr)) = %s;\n", pszAddress, pszTarget);
		fprintf(fp, "							*((const char *) psMemWrite->pUserArea + (%s - psMemWrite->lowAddr) + 1) = %s >> 8;\n", pszAddress, pszTarget);
		fprintf(fp, "						}\n");
...
void WriteValueToIo(const char *pszIoAddress, const char *pszValue)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		fprintf(fp, "checkLoop%d:\n", dwGlobalLabel);
		fprintf(fp, "		cmp	[edi], word 0ffffh ; End of our list?\n");
		fprintf(fp, "		je	WriteMacroExit%d	; Yes - ignore it!\n", dwGlobalLabel);
		fprintf(fp, "		cmp	%s, [edi]	; Are we smaller?\n", pszIoAddress);
		fprintf(fp, "		jb	nextAddr%d	; Yes... go to the next addr\n", dwGlobalLabel);
...
void ReadValueFromMemory(const char *pszAddress, const char *pszTarget)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		fprintf(fp, "		jmp	short readExit%d\n\n", dwGlobalLabel);
		fprintf(fp, "memoryRead%d:\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "						else\n");
		fprintf(fp, "						{\n");
		fprintf(fp, "							%s = *((const char *) psMemRead->pUserArea + (%s - psMemRead->lowAddr));\n", pszTarget, pszAddress);
		fprintf(fp, "						}\n");
...
void ReadWordFromMemory(const char *pszAddress, const char *pszTarget)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "						{\n");
		fprintf(fp, "							%s = *((const char *) psMemRead->pUserArea + (%s - psMemRead->lowAddr));\n", pszTarget, pszAddress);
		fprintf(fp, "							%s |= (UINT32) ((UINT32) *((const char *) psMemRead->pUserArea + (%s - psMemRead->lowAddr + 1)) << 8);\n", pszTarget, pszAddress);
		fprintf(fp, "						}\n");
		fprintf(fp, "						psMemRead = NULL;\n");
		fprintf(fp, "						break;\n");
		fprintf(fp, "					}\n");
...
void ReadValueFromIo(const char *pszIoAddress, const char *pszTarget)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
...
void JpHandler(UINT32 dwOpcode)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		else	// It's a conditional handler...
		{
			...
			fprintf(fp, "		j%s	takeJump%d	; We're going to take a jump\n", pbFlags[(dwOpcode >> 3) & 0x07], dwGlobalLabel);
			fprintf(fp, "		add	esi, 2		; Skip past the address\n");
			fprintf(fp, "		jmp	short nextInst%d	 ; Go execute the next instruction\n", dwGlobalLabel);
			fprintf(fp, "takeJump%d:\n", dwGlobalLabel);
	
			fprintf(fp, "		mov	si, [esi]	; Get our new offset\n");
			fprintf(fp, "		and	esi, 0ffffh	; Only the lower WORD is valid\n");
			fprintf(fp, "		add	esi, ebp		; Our new address!\n");
			fprintf(fp, "nextInst%d:\n", dwGlobalLabel);
			++dwGlobalLabel;
		}
...
void JrHandler(UINT32 dwOpcode)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		{
			fprintf(fp, "		sahf\n");
			fprintf(fp,     "		j%s	takeJump%d\n", pbFlags[(dwOpcode >> 3) & 0x3], dwGlobalLabel);
			fprintf(fp, "		jmp	short noJumpMan%d\n", dwGlobalLabel);
			fprintf(fp, "takeJump%d:\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
        ...
        fprintf(fp, "               sdwCyclesRemaining += 5; // Malban correction value\n");
        fprintf(fp, "               sdwAddr = (INT8) *pbPC++;   /* Get LSB first */\n");
		fprintf(fp, "				cpu.z80pc = (UINT32) pbPC - (UINT32) cpu.z80Base;\n");
		fprintf(fp, "				sdwAddr = (sdwAddr + (INT32) cpu.z80pc) & 0xffff;\n");
...
void CallHandler(UINT32 dwOpcode)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		{
			fprintf(fp, "		sahf		; Restore our flags\n");
			fprintf(fp, "		j%s	takeJump%d	; We're going call in this case\n", pbFlags[(dwOpcode >> 3) & 0x07], dwGlobalLabel);
			fprintf(fp, "		add	esi, 2		; Skip past the address\n");
			fprintf(fp, "		jmp	short noCallTaken%d	 ; Go execute the next instruction\n", dwGlobalLabel);
			fprintf(fp, "takeJump%d:\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
		...
		if (0xcd != dwOpcode)
		{
			fprintf(fp, "				if %s\n", pbFlagsC[(dwOpcode >> 3) & 0x07]);
			fprintf(fp, "				{\n");
            fprintf(fp, "                   sdwCyclesRemaining -= 7; // Malban\n");
            fprintf(fp, "                   cpu.z80pc = (UINT32) pbPC - (UINT32) cpu.z80Base;\n");
...
void RetHandler(UINT32 dwOpcode)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		{
			fprintf(fp, "		sahf\n");
			fprintf(fp, "		j%s	takeReturn%d\n", pbFlags[(dwOpcode >> 3) & 0x07], dwGlobalLabel);
			fprintf(fp, "		jmp	short retNotTaken%d\n", dwGlobalLabel);
			fprintf(fp, "takeReturn%d:\n", dwGlobalLabel);
...
void CPICPDCPIRCPDRHandler(UINT32 dwOpcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		fprintf(fp, "		jz	notBcZero%d\n", dwGlobalLabel);
		fprintf(fp, "		or	ah, 04h	; P/V set when BC not zero\n");
		fprintf(fp, "notBcZero%d:\n", dwGlobalLabel);
		...
		{
			fprintf(fp, "		sahf\n");
			fprintf(fp, "		jz	BCDone%d\n", dwRepeatOb);
			fprintf(fp, "		jnp	BCDone%d\n", dwRepeatOb);

			if (FALSE == bNoTiming)
			{
				fprintf(fp, "		sub	edi, dword 21\n");
				fprintf(fp, "		js		BCDoneExit%d\n", dwRepeatOb);
			}

			fprintf(fp, "		jmp	cpRepeat%d\n", dwRepeatOb);

			fprintf(fp, "BCDoneExit%d:\n", dwRepeatOb);
	...
	if (MZ80_C == bWhat)
	{
		...
		{
			fprintf(fp, "				sdwCyclesRemaining -= 21; // Malban  \n");
			fprintf(fp, "				if (cpu.z80A == bTemp)\n");
			fprintf(fp, "				{\n");
            fprintf(fp, "                   sdwCyclesRemaining+=5; // Malban  \n");
            fprintf(fp, "                   break;\n");
			fprintf(fp, "				}\n");
		}
        fprintf(fp, "               //}\n");
        fprintf(fp, "               }  while ((sdwCyclesRemaining >= 0) && (cpu.z80BC)); // Malban \n");
        ...
        fprintf(fp, "               }\n");
        fprintf(fp, "               else \\ MALBAN\n");
        fprintf(fp, "               {\n");
        fprintf(fp, "                   if (cpu.z80A != bTemp) sdwCyclesRemaining+=5; // only 16 cycles\n");
        fprintf(fp, "               }\n");
    }
	else
		assert(0);
...
void INIRINDRINIINDHandler(UINT32 dwOpcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		if (0xba == dwOpcode || 0xb2 == dwOpcode)
		{
			fprintf(fp, "		jz	near finalExit%d\n", dwTempLabel);
			...
			{
				fprintf(fp, "		sub	edi, dword 21\n");
				fprintf(fp, "		js		loopExit%d\n", dwTempLabel);
			}

			fprintf(fp, "		jmp	loopIt%d\n\n", dwTempLabel);
		...
		fprintf(fp, "finalExit%d:\n", dwTempLabel);	
		fprintf(fp, "		jnz	clearFlag%d\n", dwTempLabel);
		fprintf(fp, "		or	ah, 040h	; Set the Zero flag!\n");
		fprintf(fp, "		jmp	short continue%d\n", dwTempLabel);
		fprintf(fp, "clearFlag%d:\n", dwTempLabel);
	...
	if (MZ80_C == bWhat)
	{
		...
		{
            fprintf(fp, "               // Malban while ((sdwCyclesRemaining > 0) && (cpu.z80B))\n");
            fprintf(fp, "               do\n");
		}
		...
		fprintf(fp, "				cpu.z80B--;\n");
		fprintf(fp, "				}while ((sdwCyclesRemaining > 0) && (cpu.z80B)); // Malban\n");
...
void AdcSbcRegpair(UINT32 dwOpcode)
{
	...
	if (MZ80_C == bWhat)
	{
		...
		else										// SBC
		{
			...
			fprintf(fp, "				cpu.z80F |= ((dwTemp >> 16) & Z80_FLAG_CARRY);\n");
            fprintf(fp, "               cpu.z80F |= Z80_FLAG_NEGATIVE; // Malban\n");
            fprintf(fp, "               cpu.z80HL = dwTemp & 0xffff;\n");
			return;
		}
	}
...
void LDILDRLDIRLDDRHandler(UINT32 dwOpcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		if (dwOpcode == 0xb0 || dwOpcode == 0xb8)
		{
			...
			fprintf(fp, "		jmp	ldRepeat%d ; Loop until we're done!\n", dwOrgGlobal);
			fprintf(fp, "noMore%d:\n", dwGlobalLabel);
		}
	
		fprintf(fp, "		and	ah, 0e9h ; Knock out H & N and P/V\n");
		fprintf(fp, "		or		cx, cx	; Flag BC\n");
		fprintf(fp, "		jz	atZero%d ; We're done!\n", dwGlobalLabel);
	...
	if (MZ80_C == bWhat)
	{
        ...
        fprintf(fp, "               else // Malban\n");
        fprintf(fp, "               {\n");
        fprintf(fp, "                   sdwCyclesRemaining += 5; // not 21 - but 16!\n");
        fprintf(fp, "               }\n");
   ...
   char *src, *dst;
	...
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
	   switch(dwOpcode) 
		{
	   	case 0x57:  
				dst = _AL; src=_z; break;
		   case 0x5F:  
				dst = _AL; src=_z; break;
		   case 0x47:  
				dst = _z; src=_AL; break;
		   case 0x4F:  
				dst = _z; src=_AL; break;
	   }
...
void LdByteToIndex(UINT32 dwOpcode)
{
	if (MZ80_ASSEMBLY_X86 == bWhat)
	{
		...
		fprintf(fp, "		xor	dh, dh\n");
		fprintf(fp, "		or	dl, dl\n");
		fprintf(fp, "		jns	noNegate%d\n", dwGlobalLabel);
		fprintf(fp, "		dec	dh\n");
		fprintf(fp, "noNegate%d:\n", dwGlobalLabel);
...
void DataSegment()
{
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "static const char *pbPC;			/* Program counter normalized */\n");
		fprintf(fp, "static const char *pbSP;			/* Stack pointer normalized */\n");
		...
		fprintf(fp, "static const char *pbAddAdcTable;	/* Pointer to add/adc flag table */\n");
		fprintf(fp, "static const char *pbSubSbcTable;	/* Pointer to sub/sbc flag table */\n");
...
void EmitRegularInstructions()
{
	...
	if (MZ80_C == bWhat)
	{
		...
		while (dwLoop < 0x100)
		{
			...
			{
				fprintf(fp, "				sdwCyclesRemaining -= %d;\n", bTimingRegular[dwLoop]);
			}
...
void EmitDDInstructions()
{
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "void DDFDCBHandler(UINT32 dwWhich)\n");
		fprintf(fp, "{\n");
        fprintf(fp, "   if (dwWhich) // Malban\n");
        fprintf(fp, "   {\n");
        fprintf(fp, "       dwAddr = (UINT32) ((INT32) cpu.z80IY + ((INT32) ((signed char)(*pbPC++))   )) & 0xffff;\n");
        fprintf(fp, "   }\n");
        fprintf(fp, "   else\n");
        fprintf(fp, "   {\n");
        fprintf(fp, "       dwAddr = (UINT32) ((INT32) cpu.z80IX + ((INT32) ((signed char)(*pbPC++))  )) & 0xffff;\n");
        fprintf(fp, "   }\n\n");
...
void InitCode(void)
{
	...
	if (MZ80_C == bWhat)
	{
		...
		fprintf(fp, "	const char *pbTempPtr;\n");
		fprintf(fp, "	const char *pbTempPtr2;\n");
        ...
        fprintf(fp, "           { // Malban\n");
        fprintf(fp, "           int n,c,h,i;\n");
        fprintf(fp, "           for (n=0;n<2;n++)\n");
        fprintf(fp, "           {\n");
        fprintf(fp, "               for (c=0;c<2;c++)\n");
        fprintf(fp, "               {\n");
        fprintf(fp, "                   for (h=0;h<2;h++)\n");
        fprintf(fp, "                   {\n");
        fprintf(fp, "                       for (i=0;i<256;i++)\n");
        fprintf(fp, "                       {\n");
        fprintf(fp, "                           int adr = (c+(h<<1)+(n<<2)) << 8;\n");
        fprintf(fp, "                           int fValue = 0;\n");
        fprintf(fp, "                           int value = i;\n");
        fprintf(fp, "                           int t;\n");
        fprintf(fp, "                           unsigned char a;\n");
        fprintf(fp, "                           unsigned char flag =0;\n");
        fprintf(fp, "                           flag |= (n==0?0:Z80_FLAG_NEGATIVE);\n");
        fprintf(fp, "                           flag |= (c==0?0:Z80_FLAG_CARRY);\n");
        fprintf(fp, "                           flag |= (h==0?0:Z80_FLAG_HALF_CARRY);\n");
        fprintf(fp, "                          t=0;\n");
        fprintf(fp, "                          if((flag&Z80_FLAG_HALF_CARRY) || ((value & 0xF) > 9) )\n");
        fprintf(fp, "                                t++;\n");
        fprintf(fp, "                          if((flag&Z80_FLAG_CARRY) || (value > 0x99) )\n");
        fprintf(fp, "                          {\n");
        fprintf(fp, "                                t += 2;\n");
        fprintf(fp, "                                flag |= Z80_FLAG_CARRY;\n");
        fprintf(fp, "                          }\n");
        fprintf(fp, "                          // builds final H flag\n");
        fprintf(fp, "                          if ((flag&Z80_FLAG_NEGATIVE) && !(flag&Z80_FLAG_HALF_CARRY))\n");
        fprintf(fp, "                             flag &=~(Z80_FLAG_HALF_CARRY);\n");
        fprintf(fp, "                          else\n");
        fprintf(fp, "                          {\n");
        fprintf(fp, "                              if ((flag&Z80_FLAG_NEGATIVE) && (flag&Z80_FLAG_HALF_CARRY))\n");
        fprintf(fp, "                                 if (((value & 0x0F)) < 6) flag |= Z80_FLAG_HALF_CARRY;\n");
        fprintf(fp, "                              else\n");
        fprintf(fp, "                                 if ((value & 0x0F) >= 0x0A) flag |= Z80_FLAG_HALF_CARRY;\n");
        fprintf(fp, "                          }\n");
        fprintf(fp, "                          switch(t)\n");
        fprintf(fp, "                          {\n");
        fprintf(fp, "                               case 1:\n");
        fprintf(fp, "                                   value += (flag&Z80_FLAG_NEGATIVE)?0xFA:0x06; // -6:6\n");
        fprintf(fp, "                                   break;\n");
        fprintf(fp, "                               case 2:\n");
        fprintf(fp, "                                   value += (flag&Z80_FLAG_NEGATIVE)?0xA0:0x60; // -0x60:0x60\n");
        fprintf(fp, "                                   break;\n");
        fprintf(fp, "                               case 3:\n");
        fprintf(fp, "                                   value += (flag&Z80_FLAG_NEGATIVE)?0x9A:0x66; // -0x66:0x66\n");
        fprintf(fp, "                                   break;\n");
        fprintf(fp, "                          }\n");
        fprintf(fp, "                          if ((value & 128)) flag|=Z80_FLAG_SIGN;\n");
        fprintf(fp, "                          if (value==0) flag|=Z80_FLAG_ZERO;\n");
        fprintf(fp, "                          if (value & 32) flag|=Z80_FLAG_UNDEFINED2;\n");
        fprintf(fp, "                          if (value & 8) flag|=Z80_FLAG_UNDEFINED1;\n");
        fprintf(fp, "                          if (parity((unsigned char)i)) flag|=Z80_FLAG_OVERFLOW_PARITY;\n");
        fprintf(fp, "                          a = (unsigned char) (value&0xff);\n");
        fprintf(fp, "                           adr = adr | ((unsigned char) i);\n");
        fprintf(fp, "                           fValue = (flag<<8) | ((unsigned char) (value&0xff));\n");
        fprintf(fp, "                           wDAATable[adr] = fValue;\n");
        fprintf(fp, "                       }\n");
        fprintf(fp, "                   }\n");
        fprintf(fp, "               }\n");
        fprintf(fp, "           }}\n");
