...
void mon(char *s) {
  ...
  fflush(stderr);
  fprintf(stderr,
          "%04x: A=%02x B=%02x X=%04x Y=%04x S=%04x U=%04x CC=%02x DP=%02x E%cF%cH%cI%cC%cZ%cV%cN%c   %s\n",
          PC,
          A, B, X, Y, S, U, CC,
          (UINT8)(DP>>8),
          E+'0', F+'0', (((H & 0x10) != 0)?1:0)+'0', I+'0', (((C & 0x100) != 0)?1:0)+'0', ((Z != 0)?1:0)+'0', (((SINT8)V < 0)?1:0)+'0', (((SINT8)N < 0)?1:0)+'0',
          s);
  fflush(stderr);
}
...
#include "cputest.c"  // or whichever program has been translated...

    // these two interceptions are for the I/O support in cputest.c: 
...
int main (int argc, char **argv) {
  ...
  for (;;) {
    //sbt();       // sbt returns if there is no entrypoint for value of PC
    emulator();  // emulator returns when there *is* an entrypoint for the value of PC
                 // We keep an array of flag bits anyway, so might as well reuse here!
  }
}
