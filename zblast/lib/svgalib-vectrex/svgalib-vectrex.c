 ...
 */
void donotusethis__svgalib_vectrex_recalcheck(void)
{
...
{
	return ((int)((coordinate / (float)infotable[vgamode].xdim) * (65535/2)) - (32767/2) );
}

int __svgalib_vectrex_scaleycoordinate(int coordinate)
{
	return ((~((int)((coordinate / (float)infotable[vgamode].ydim) * (65535/2)) - (32767/2))) + 1);
}
...
// MALBAN	__svgalib_vectrex_recalcheck();
#ifdef PITREX_DEBUG
