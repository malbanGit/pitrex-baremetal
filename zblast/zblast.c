...
unsigned char shipHit_data[];
unsigned char shipHit2_data[];
unsigned char CreateEnemyShip_data[];
unsigned char opponentShot_data[];
unsigned char enemyBigShot_data[];
unsigned char pncShot_data[];
unsigned char somethingAppears_data[];
unsigned char nemesis_4_data[];
unsigned char levelEnd_data[];
...
static int zblastIniHandler(void* user, const char* section, const char* name, const char* value)
{
  // cascading ini files
  ...
  if (MATCH_NAME("INGAME_MUSIC")) inGameMusic = atoi(value);
  if (MATCH_NAME("INGAME_SFX")) inGameSFX = atoi(value);
  else 
  {
  }
/*
...
//  v_loadSettings("zblast", settingsBlob, SETTINGS_SIZE);
    ...
    int pv = 0;
    if ((pv = ini_parse("zblast.ini", zblastIniHandler, 0)) < 0) 
    {
          printf("zblast.ini not loaded!\n\r");
    }
    else
          printf("zblast.ini loaded!\n\r");
...
*/
/* Vectrex Heading: */
v_printString(-100, 60, "ZBLAST", 36, 90);
...
centretextsize(475,bigwindow?3:1,
  ...
  if(!paused && !dead)
    ...
    if(keypress_pdc && waspdc==0 && pdc>0)
      ...
      queuesam(PSHOT_CHANNEL,PWAVE_SAMPLE);
        ...
        if (inGameSFX) v_playSFXStart(pncShot_data, 0, 0);
      firemany(ypos);
//      showstatus(0);
      }
    else
    ...
    if(keypress_fire)
      ...
      if(wasfire<=0)
        ...
        if (inGameSFX) v_playSFXStart(Ast_PlayerShot_data, 0, 0);
    ...
    if(shield)
      ...
      if(deadyet(xpos,ypos))
        ...
        if (inGameSFX) v_playSFXStart(nemesis_4_data, 2, 0);
          }
  //      showstatus(0);
        }
    }
    ...
    if(tweenwave)
      ...
      if(!dead && (count=countbaddies())==0)
        ...
        queuesam(EFFECT_CHANNEL,LVLEND_SAMPLE);
         ...
         if (inGameSFX) v_playSFXStart(levelEnd_data, 2, 0);
...
// due to vector display - we must show the status always!
      showstatus(0);

    if (ymloaded) v_playYM();      
    v_playAllSFX();
    v_doSound(); // not needed in irqMode
    v_WaitRecal();
...
if(wavenum>1 || looped)
  ...
  if(lostlife==0)
    {
    vga_setcolor(showthem?14:0);
    centretext(267,"No hits from wave - energy increased 10");
    }

  if(lostlife<10)
    {
    vga_setcolor(showthem?3:0);
    centretext(321,"Bonus - extra PDC");
...
//      centretext(374,"All Levels Complete - energy increased 40");
      centretext(374,"All Complete - energy increased 40");
    else
//      centretext(374,"Level Complete Bonus - energy increased 10");
      centretext(374,"Bonus - energy increased 10");
    }
  }
...
vgadrawtext(bigwindow?540:270,0,bigwindow?3:2,buf);
sprintf(buf,"%03d",pdc);
        ...
        if((shot[g].x>=x1)&&(shot[g].x<=x2)&&
          ...
          if(baddie[f].hits==0)
            ...
            switch(baddie[f].which)
              ...
              case  1: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case  3: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case  4: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case  5: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case  6: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case 10: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
              case 11: if (inGameSFX) v_playSFXStart(Ast_SaucerAstExplosion_data, 1, 0); break;
/*
...
*/              
            ...
            switch(baddie[f].which)
              {
              case  1: if (inGameSFX) v_playSFXStart(shipHit_data, 1, 0); break;
              case  3: if (inGameSFX) v_playSFXStart(shipHit2_data, 1, 0); break;
              case  4: if (inGameSFX) v_playSFXStart(shipHit_data, 1, 0); break;
              case  5: if (inGameSFX) v_playSFXStart(shipHit2_data, 1, 0); break;
              case  6: if (inGameSFX) v_playSFXStart(shipHit_data, 1, 0); break;
              case 10: if (inGameSFX) v_playSFXStart(shipHit2_data, 1, 0); break;
              case 11: if (inGameSFX) v_playSFXStart(shipHit_data, 1, 0); break;
/*
...
if((n=findavailablebaddie())!=-1)
  ...
  baddie[n].hitnow=0;
  if(which==2)
        if (inGameSFX) v_playSFXStart(CreateEnemyShip_data, 0, 0);
...
// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter/shipHit.afx"
unsigned char shipHit_data[]={
  0x7F, 0x11, 0x00, 0x11, 0x7F, 0x12, 0x00, 0x12, 0x7F, 0x13,
  0x00, 0x13, 0x7E, 0x14, 0x00, 0x14, 0x7E, 0x15, 0x00, 0x15,
  0x7D, 0x16, 0x00, 0x16, 0x7D, 0x17, 0x00, 0x17, 0x7B, 0x18,
  0x00, 0x18, 0x7B, 0x19, 0x00, 0x19, 0x7A, 0x1A, 0x00, 0x1A,
  0x7A, 0x1B, 0x00, 0x1B, 0x79, 0x1C, 0x00, 0x1C, 0x79, 0x1D,
  0x00, 0x1D, 0x78, 0x1E, 0x00, 0x1E, 0x78, 0x1F, 0x00, 0x1F,
  0xD0, 0x20};//

// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter/shipHit2.afx"
unsigned char shipHit2_data[]={
  0x6F, 0x57, 0x00, 0x06, 0x4E, 0x0C, 0x4D, 0x12, 0x4B, 0x18,
  0x4A, 0x16, 0x49, 0x1C, 0x48, 0x02, 0x47, 0x08, 0x46, 0x0E,
  0x45, 0x14, 0x44, 0x1A, 0x43, 0x00, 0x42, 0x0B, 0x41, 0x11,
  0x41, 0x17, 0xD0, 0x20};//

// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter/CreateEnemyShip.afx"
unsigned char CreateEnemyShip_data[]={
  };//

// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter/opponentShot.afx"
unsigned char opponentShot_data[]={
  0xEE, 0x16, 0x02, 0x10, 0xAD, 0x2C, 0x02, 0xAC, 0x42, 0x02,
  0xAB, 0x58, 0x02, 0xAA, 0x6E, 0x02, 0xA9, 0x84, 0x02, 0xA8,
  0x9A, 0x02, 0xA7, 0xB0, 0x02, 0xA6, 0xC6, 0x02, 0xA5, 0xDC,
  0x02, 0xA4, 0xF2, 0x02, 0xA3, 0x08, 0x03, 0xD0, 0x20};//

// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter/enemyBigShot.afx"
unsigned char enemyBigShot_data[]={
  0x6F, 0x20, 0x00, 0x0A, 0x2E, 0x00, 0x01, 0x2D, 0x80, 0x01,
  0x2F, 0x20, 0x00, 0x2E, 0x00, 0x01, 0x2D, 0x80, 0x01, 0x2C,
  0x00, 0x02, 0x6F, 0x25, 0x00, 0x1E, 0x2F, 0x1A, 0x00, 0x2F,
  0x15, 0x00, 0x2E, 0x10, 0x00, 0x2E, 0x2A, 0x00, 0x2D, 0x20,
  0x00, 0x2D, 0x10, 0x00, 0x2C, 0x0A, 0x00, 0x2C, 0x25, 0x00,
  0x2B, 0x1A, 0x00, 0x2B, 0x15, 0x00, 0x2A, 0x10, 0x00, 0x2A,
  0x2A, 0x00, 0x2A, 0x20, 0x00, 0x29, 0x10, 0x00, 0x29, 0x0A,
  0x00, 0x29, 0x25, 0x00, 0x29, 0x1A, 0x00, 0xD0, 0x20};//
// AYFX - Data of file[]={ "/Users/chrissalo/NetBeansProjects/Vide/projects/Choplifter