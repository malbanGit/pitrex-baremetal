#define MAX_YM_SIZE (500*1000)

char ymBuffer[MAX_YM_SIZE]; // max 500k ym buffer
char ymBufferLoad[MAX_YM_SIZE]; // max 500k ym buffer
...
//		     printf("%i = %i ",(outer+out_counter*14), poker);
    ...
    {
        vbl_len = 0;
        unsigned char *buf = (unsigned char *) ymBufferLoad;
        unsigned char *out_buf = (unsigned char *) ymBuffer;
        int len=fsize;        
        char format[5];
        format[0] = (char)buf[0];
        format[1] = (char)buf[1];
...
outputCurrentDirectory();
    ...
    */
      printf("Loading: %s \r\n", FILE_NAME);
      unsigned int fsize = MAX_YM_SIZE-1;
	...
	v_initYM((unsigned char *)ymBuffer, length, 1);      
