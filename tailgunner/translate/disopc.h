...
unsigned int printMode(char *dest, unsigned int mode, unsigned int adr, unsigned int useBreg)
{
  ...
  switch (mode)
  {
    ...
    case AIM4:
    case IM4:
      sprintf(OpcBfr, "#$%X", ImmOpd = (rom[adr] & 0x0F));
      break;

      /* display 4 bit extended immediate value */

    case AIM4X:	
      sprintf(OpcBfr, "#$%X", ImmOpd = ((rom[adr] & 0x0F) << 8));
      break;

      /* display 8 bit immediate value */

    case AIM8:
      sprintf(OpcBfr, "#$%02X", ImmOpd = (rom[adr+1]));
    ...
    case AIMX:
      ...
      sprintf(OpcBfr, "#$%03X", ImmOpd = (ii));	/* print to temperary buffer */
      break;
