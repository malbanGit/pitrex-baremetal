...
{
    return USBKeyboardIsDownPhysical(s_pLibrary->pUKBD1, key);
}
int USPiKeyboardIsDown(unsigned char key)
{
   return USBKeyboardIsDown(s_pLibrary->pUKBD1, key);
}
/*
#define LCTRL       (1 << 0)
#define LSHIFT      (1 << 1)
...
{
    return USBKeyboardDeviceGetModifiers(s_pLibrary->pUKBD1);
}
unsigned char USPiKeyboardToPhysical(unsigned char key)
{
   return USBKeyboardToPhysical(s_pLibrary->pUKBD1, key);
}

const char *USPiKeyboardPhysicalToKeyboard(unsigned char phys, unsigned char ucModifiers)
{
    return USBKeyboardPhysicalToKeyboard(s_pLibrary->pUKBD1, phys, ucModifiers);
}
...
{
    return USBKeyboardLastPhysicalKeyDown();
}
unsigned char USPiKeyboardLastKeyDown()
{
  unsigned char p = USBKeyboardLastPhysicalKeyDown();
  const char *s = USPiKeyboardPhysicalToKeyboard(p, USPiKeyboardGetModifiers());
  if (s==0) return 0;
  return *s;
}
