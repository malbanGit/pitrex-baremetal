...
int physicalKeyDown[127]; // map for "physical" 1 = pressed, 0 = no pressed
...
{
	assert (pThis != 0);
  // I want to be able to set a 0 handler!!!
...
static unsigned char lastPhysicalKeyDown = 0;
static int oldReportBuffer[BOOT_REPORT_SIZE];
void USBKeyboardDeviceCompletionRoutine (TUSBRequest *pURB, void *pParam, void *pContext)
{
	...
	{
        // init the keyboard status map
        for (int i=0+2;i<BOOT_REPORT_SIZE; i++)
        {
          physicalKeyDown[oldReportBuffer[i]] = 0; // all keys that were before down -> up
          oldReportBuffer[i] = pThis->m_pReportBuffer[i]; // remember the new values as old
          physicalKeyDown[pThis->m_pReportBuffer[i]] = 1; // all keys that are now down -> down
        }
...
// TODO: build a reverse translation table
// only shift and normal
// not CTRL / ALT
extern const u16 s_DefaultMap[PHY_MAX_CODE+1][K_ALTSHIFTTAB+1];

unsigned char USBKeyboardToPhysical(TUSBKeyboardDevice *pThis, unsigned char key)
{
  unsigned char physCode = 0; 
  
  for (int i=0;i<0x7f;i++)
  {
    if ((key == s_DefaultMap[i][0]) || (key == s_DefaultMap[i][1]))// normal // shift  -> I don't care about other modes
    {
      return physCode = i;
    }
  }
...
int USBKeyboardIsDown (TUSBKeyboardDevice *pThis, unsigned char key)
{
  return USBKeyboardIsDownPhysical(pThis, USBKeyboardToPhysical(pThis, key));
}


// key in pyhsical
int USBKeyboardIsDownPhysical(TUSBKeyboardDevice *pThis, unsigned char phys)
{
  if (phys == 0) return 0;
...
// no modifiers!
// might return 0! -> key not found1
const char *USBKeyboardPhysicalToKeyboard(TUSBKeyboardDevice *pThis, unsigned char phys, unsigned char ucModifiers)
{
    char Buffer[2];
    u16 usLogCode = KeyMapTranslate (&pThis->m_KeyMap, phys, ucModifiers);
    const char *pKeyString = KeyMapGetString (&pThis->m_KeyMap, usLogCode, ucModifiers, Buffer);

    return pKeyString;
}

// returns the last pressed char
// ONCE after the key press was handled by the hadnler
// USBKeyboardDeviceCompletionRoutine is done
// after that it will return 0, even if the key is still pressed!
unsigned char USBKeyboardLastPhysicalKeyDown()
{
  unsigned char t = lastPhysicalKeyDown;
  lastPhysicalKeyDown = 0;
  return t;
}