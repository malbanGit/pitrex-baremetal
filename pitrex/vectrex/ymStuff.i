    ...
    //////////////////////////////////////
    static int vbl_len;
    static int convert_ym3(unsigned char *out_buf, unsigned char *in_buf, int len)
    {
        int pos=4;
        int todo=((len-4)/14);
        vbl_len=todo;
...
//        song_name = file_name;
    ...
    static int convert_ym2(unsigned char *out_buf, unsigned char *in_buf, int len)
    {
        return convert_ym3(out_buf, in_buf, len);
    }

    static int convert_ym3b(unsigned char *out_buf, unsigned char *in_buf, int len)
    {
         return convert_ym3(out_buf, in_buf, len);
    }

    static int convert_ym4(unsigned char *out_buf, unsigned char *in_buf, int len)
    {
        int c1;
        int c2;
        int c3;
...
//        tl.printMessageSU(tab("Comment", DATA_TAB)+comment+"");
    ...
    static int convert_ym5(unsigned char *out_buf, unsigned char *in_buf, int len)
    {
//	printf("in buf at = %X!\r\n",in_buf);
//	printf("out buf at = %X!\r\n",out_buf);
...
printf ("YM final song mem used: %i\n\r", out_counter*14);
        return vbl_len;
    }

    static int initym(int fsize,unsigned char *out_buf, unsigned char *buf)
    {
        vbl_len = 0;
        int len=fsize;        
        char format[5];
...
int v_loadYM(unsigned char *FILE_NAME, int len ,char *dest, char *loadbuf)
{
    ...
    FIL file_object_rd;
    rc_rd = f_open(&file_object_rd, (const char*)FILE_NAME, (unsigned char) FA_READ);
int length = 0;
    if (rc_rd != FR_OK)
    {
    ...
    */
      printf("Loading: %s \r\n", FILE_NAME);
      unsigned int fsize = len;
      rc_rd = f_read(&file_object_rd, loadbuf, fsize, &fsize);
      if ( rc_rd!= FR_OK)
      {
      ...
      else
      {
          ...
          length = initym(fsize, (unsigned char *)dest, (unsigned char *)loadbuf);
      }
    }
	return length;
}
