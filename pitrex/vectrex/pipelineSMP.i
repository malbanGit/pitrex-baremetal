...
/*
 static VectorPipeline *pMem = (VectorPipeline *)(CORE1_NON_SHAREABLE+0x20000000); // 1 MB
 memcpy(pMem,dpl, MAX_PIPELINE*sizeof(VectorPipeline));
 printf("dpl[c].type: %i - pMem[c].type: %i (size:%i)\n", dpl[c].type, ((VectorPipeline*)pMem)[c].type, MAX_PIPELINE*sizeof(VectorPipeline));  
 dpl = pMem;

 
 static VectorPipeline *pMem = (VectorPipeline *)(CORE1_NON_SHAREABLE); // 1 MB
 memcpy(pMem,dpl, MAX_PIPELINE*sizeof(VectorPipeline));
 printf("dpl[c].type: %i - pMem[c].type: %i (size:%i)\n", dpl[c].type, ((VectorPipeline*)pMem)[c].type, MAX_PIPELINE*sizeof(VectorPipeline));  
 dpl = pMem;
...
//currentSwitch = dpl[c].type;
//currentRead = vectorPipeLineIsRead;
//currentStage = -1;
...
//currentT1=-1;
        ...
        if (dpl[c].flags & PL_T1_LO_EQUALS)
        {
          ...
          setCounter1Mark(); 
//currentT1=((dpl[c].this_timing)>>8)+0x80000000;
        }
        else
        {
          // resets  IRQ Flag
