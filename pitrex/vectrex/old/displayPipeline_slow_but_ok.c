...
/////////// new placement         
        if (delayedBeamOff)
        {
          LINE_DEBUG_OUT("     DELAYED BEAM OFF (%i)\r\n", dpl[c].y);
// ??          if (dpl[c].type == PL_DRAW) toDelayOff-=5; // consecutive draws wait less ;-)
//          waitT1OffMark(DELAY_AFTER_T1_END_VALUE-4);
          waitT1OffMark(DELAY_AFTER_T1_END_VALUE);

//wait delayed beam off can not be done after Y setting!          
          // e
// not better          
...
//wait delayed beam off can not be done after Y setting!          
          // e
//ss swaitUntilAndUse(50*ONE_WAIT_UNIT);
          SWITCH_BEAM_OFF4();
        }
*/
...
//waitUntilAndUse(5*ONE_WAIT_UNIT);
